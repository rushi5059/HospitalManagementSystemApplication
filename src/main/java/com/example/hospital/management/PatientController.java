...
public class PatientController {
    ...
    public String addPatient(@RequestParam("patientId")Integer patientId, @RequestParam("name")String name, @RequestParam("age")Integer age, @RequestParam("disease")String disease){
        ...
        return "Patient added successfully via RequestParam!";
    }
    @PostMapping("/addtPatientViaRequestBody")
    ...
    @GetMapping("/getOver50Patient")
    public List<Patient> getOver50Patient(@RequestParam("age") Integer age){
        List<Patient> patients = new ArrayList<>();
        for(Patient p : patientDb.values()){
            if(p.getAge() > age){
                patients.add(p);
